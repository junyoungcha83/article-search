...
설정의 **이번 달 비용 한도(USD)** 에 금액을 넣어 두면, 이번 달 누적이 그 금액을 넘었을 때
검색·요약을 보내기 **전에** 물어본다. 실수로 눌러 요금이 나가는 것을 막는 장치다.

- 막지는 않고 물어본다 — '알아요, 계속' 을 고르면 그대로 진행한다
- 저장된 결과를 다시 볼 때는 요금이 들지 않으므로 **묻지 않는다**
- 비워 두면 확인하지 않는다
- 이번 달 누적·남은 금액은 **ℹ️ 정보** 탭에 표시된다
- 기준은 현지 시각 기준 이달 1일 0시. 지난달 기록은 누적에서 빠진다

앱 차원의 장치라 이 기기에만 적용된다. 계정 전체를 막으려면
[콘솔의 사용 한도](https://console.anthropic.com/settings/limits)를 함께 걸어 두는 편이 확실하다.
