...
- 검색 결과는 로컬에 저장되고 **다시 열면 저장본을 먼저 보여준다**(요금 0원). 저장된 지 6시간이 지났으면
  `askRefresh()` 팝업으로 **새로 검색할지 물어보고**, '예'를 눌렀을 때만 API를 호출한다.
  결과 위 안내줄의 `새로 검색` 버튼으로도 강제 재검색(`runSearch({force:true})`)이 가능하다.
